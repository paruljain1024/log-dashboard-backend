...
public class TypeNameMapper {
    ...
    private volatile Properties effectiveProperties =
            new Properties();

    private final Path externalFilePath;

    private volatile boolean externalFilePresent =
            false;

    private volatile long externalFileLastModified =
            -1L;
    ...
    private void refreshMappingsIfChanged() {
        ...
        long modified =
                present
                        ? externalFile.lastModified()
                        : -1L;

        if (present == externalFilePresent
                && modified == externalFileLastModified) {
    ...
    private void reloadMappings() {
        ...
        if (externalFile.isFile()) {
            ...
            try (FileInputStream fis =
                ...
                externalFileLastModified =
                        externalFile.lastModified();

                System.out.println(
                        "Loaded external type mapping from: "
                                + externalFilePath
                );
                ...
                System.out.println(
                        "Failed to load external type mapping from: "
                                + externalFilePath
                );
            ...
            System.out.println(
                    "External type mapping not found at: "
                            + externalFilePath
